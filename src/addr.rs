...
impl VirtualAddress {
    ...
    pub const fn new_canonical(addr: usize) -> VirtualAddress {
        VirtualAddress(canonicalize_virtual_address(addr as u64))
    }
...
#[inline]
const fn canonicalize_virtual_address(virt_addr: u64) -> usize {
    ...
    ((virt_addr << 16) as i64 >> 16) as usize
}

#[inline]
fn is_canonical_physical_address(phys_addr: u64) -> bool {
    let addr_prefix = (phys_addr >> 52) & (0xFFF);
    match addr_prefix {
        0 => true,
        _ => false,
    }
